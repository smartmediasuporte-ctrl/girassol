...
import { asaasProvider } from "@/lib/payments";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const evt = await asaasProvider.parseWebhook(body, req.headers);
    if (!evt) return NextResponse.json({ ok: true, ignored: true });

    const order = await findOrderByCharge("asaas", evt.external_charge_id);
